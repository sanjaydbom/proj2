...
// Security types
#define CLIENT_HELLO 0x80
#define VERSION_TAG  0x81
#define NONCE        0x82
#define PUBLIC_KEY   0x83

#define SERVER_HELLO 0x90
#define HANDSHAKE_SIGNATURE 0x91

#define CERTIFICATE 0xA0
#define DNS_NAME    0xA1
#define SIGNATURE   0xA2
#define LIFETIME    0xA3

#define DATA        0xC0
#define IV          0xC1
#define MAC         0xC2
#define CIPHERTEXT  0xC3
...
static inline void print(const char* txt) { fprintf(stderr, "%s\n", txt); }
...
static inline tlv* deserialize_tlv(uint8_t* buf, uint16_t size) {
    ...
    uint8_t type = *buf;
    buf += 1;
    if (buf - buffer >= size) return NULL;
    
    tlv* t = create_tlv(type);
    if (*buf == VN3) {
        buf += 1;
        if (buf - buffer + 1 >= size) return NULL;
    ...
    if (buf - buffer + t->length > size) return NULL;

    // Recursive parsing for container types
    if (type == CLIENT_HELLO || type == SERVER_HELLO || type == CERTIFICATE || type == DATA) {
        uint8_t* buf2 = buf;
        for (int i = 0; i < MAX_CHILDREN; i++) {
            if ((buf - buf2) == t->length) break;
            tlv* child = deserialize_tlv(buf, t->length - (buf2 - buf));
            if (child == NULL) return NULL;
            t->children[i] = child;
            buf += 1;
            if (buf - buf2 >= t->length) return NULL;
...
static inline tlv* get_tlv(tlv* t, uint8_t type) {
    ...
    for (int i = 0; i < MAX_CHILDREN; i++) {
        if (t->children[i] != NULL && t->children[i]->type == type) return t->children[i];
    }
    for (int i = 0; i < MAX_CHILDREN; i++) {
...
static inline void print_tlv_bytes(uint8_t* buffer, uint16_t len) {
    ...
    while (buf - buffer < len) {
        ...
        buf += 1;
        if (buf - buffer >= len) { print("MALFORMED"); return; }
        uint16_t length = 0;
        if (*buf == VN3) {
            buf += 1;
            if (buf - buffer + 1 >= len) { print("MALFORMED"); return; }
            length = ntohs(*((uint16_t*) buf));
            buf += 2;
        } else {
            length = *buf;
            buf++;
            if (buf - buffer + length > len) { print("MALFORMED"); return; }
        }
